...
export const useTotalBalance = () => {
  ...
  const fetchBalance = async (skipLoading = false) => {
    try {
      if (!skipLoading) {
        setLoading(true);
      }
  ...
  useEffect(() => {
    ...
    const setupSubscription = async () => {
      ...
      const channel = supabase
        ...
        .on(
          ...
          async (payload) => {
            console.log('Mudança detectada no total balance:', payload.eventType, payload);
            // Recarregar saldo imediatamente quando houver mudanças
        ...
        .subscribe((status) => {
          if (status === 'SUBSCRIBED') {
            console.log('✅ Real-time subscription ativa (total balance)');
    ...
    const pollingInterval = setInterval(() => {
      fetchBalance(true); // skipLoading para não mostrar loading
    }, 5000); // 5 segundos
